"""Oulu2026 Visitor Flow Framework, forecast package.

Reads the canonical tables in ``data/processed/`` and writes venue-level daily and
hourly forecasts, backtest metrics and the empirical prediction intervals described
in ``docs/FRAMEWORK_PLAN.md`` chapter 8. Nothing here touches the network.

The structured logger is duplicated from ``ovf_ingest`` on purpose: the two packages
share a file contract, not code, so the forecast dependency group stays limited to
pandas, numpy, scikit-learn and pyarrow.
"""
